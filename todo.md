...
### 2026-06-02 — Phase 4a: アプリ内完結レポート作成画面（基盤）
指示書 `phase4a-instructions.md` に基づき、Notion運用レポートFMT準拠のレポート作成画面を新設。「数値テーブル+基本グラフ（自動）+ AI考察（編集・保存可）+ 過去月数値の手打ち入力」まで。デザイン作り込み・特殊グラフ・サムネイル・エクスポートは 4b/4c へ。
- 新画面: `/admin/eval/report`（既存 `/admin/eval/generate` は無変更で温存）
- 新API: `/api/report-builder/` 配下4本（generate-section / save / manual-kpi / history）
- 生成: `eval_prompt_versions` の is_active 最新を自動選択（follower=v1.6/他=v1）+ 前セクション文脈引き継ぎ。eval_runs には書かない
